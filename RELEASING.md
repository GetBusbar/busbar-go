...
This repo has two separate, deliberately-decoupled mechanisms: keeping `main`
in sync with busbarAI, and cutting a version tag. Don't confuse them.

## 1. `main` is auto-synced with busbarAI's `dev` branch
...
workflow run fails loudly and does **not** push a broken commit — `main`
stays on the last known-good sync until someone fixes it by hand.

This means **`main` tracks `dev`, which is unstable by definition.** A commit
landing on `main` via this workflow is not a release and does not mean the
SDK is ready to depend on at a pinned point — it means the generated code is
consistent with *some* snapshot of busbarAI's in-progress spec.
...
**Cut a tag by hand when busbarAI itself ships a real, tagged release** (i.e.
a `main`/release build of busbarAI at a specific `info.version`, not just a
`dev` snapshot):

1. Confirm `busbar-go`'s `main` is already synced to the spec matching that
...
2. Update the "Versioning" section of `README.md` if the reported
   `info.version` changed.
3. `git tag vX.Y.Z -m "busbar-go vX.Y.Z — generated from busbar <version> spec"`
4. `git push origin vX.Y.Z` — this triggers `.github/workflows/release.yml`,
